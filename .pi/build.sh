...
# docker buildx build --tag investodemo --file .pi/Dockerfile --platform linux/arm64 --cache-from investodemo:cache --cache-to investodemo:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar investodemo
...
sudo bash -c "echo investodemo > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/investodemo.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/investodemo.img
